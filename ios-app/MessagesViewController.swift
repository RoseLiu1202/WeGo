...
class MessagesViewController: MSMessagesAppViewController {
    ...
    private let chatId = "z1wwkb5jUwdw5H98oLq3" // Your hardcoded chat ID
...
//        showIntroThenPoll()
    ...
    private func setupTableView() {
        ...
        tableView.register(PollMessageCell.self, forCellReuseIdentifier: "PollMessageCell")
        tableView.register(ThaiPollMessageCell.self, forCellReuseIdentifier: "ThaiPollMessageCell")
        tableView.register(NewPollMessageCell.self, forCellReuseIdentifier: "NewPollMessageCell")
...
extension MessagesViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Simple! If text is "POLL", show poll
        if message.text == "POLL" {
            let cell = tableView.dequeueReusableCell(withIdentifier: "PollMessageCell", for: indexPath) as! PollMessageCell
            return cell
        }
        if message.text == "POLL_THAI" {
            let cell = tableView.dequeueReusableCell(withIdentifier: "ThaiPollMessageCell", for: indexPath) as! ThaiPollMessageCell
            return cell
        }
        if message.text == "POLL_NEW" {
            let cell = tableView.dequeueReusableCell(withIdentifier: "NewPollMessageCell", for: indexPath) as! NewPollMessageCell
            return cell
        }
...
extension MessagesViewController {
    ...
    @objc func showStuckThenPoll() {
        // Step 1️⃣: Bot notices they're stuck
        let summaryMsg = ChatMessage(
            text: "Seems like we are stuck, here is a summary of what you guys have talked about so far, maybe take a vote?",
            sender: "wego-bot",
            senderName: "WeGo Bot"
        )
        chatSession.messages.append(summaryMsg)
        tableView.reloadData()
        scrollToBottom(animated: true)

        // Step 2️⃣: After a short pause, show new poll
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            let newPollMsg = ChatMessage(
                text: "POLL_NEW",
                sender: "wego-bot",
                senderName: "WeGo Bot"
            )
            self.chatSession.messages.append(newPollMsg)
            self.tableView.reloadData()
            self.scrollToBottom(animated: true)
        }
    }
    
    @objc func showPollTest() {
        let roseMsg = ChatMessage(
            text: "Thanks WeGo Bot",
            sender: "rose",
            senderName: "Rose"
        )
        chatSession.messages.append(roseMsg)
        
        let pollMsg = ChatMessage(
            text: "POLL",  // This magic word shows the poll!
            sender: "wego-bot",
            senderName: "WeGo Bot"
        )
        chatSession.messages.append(pollMsg)
        
        tableView.reloadData()
        scrollToBottom(animated: true)
    }
    
    @objc func showIntroThenPoll() {
        // 1️⃣ Create a system-like "centered" WeGo Bot message
        let introMsg = ChatMessage(
            text: "Looks like Rose needs some help, don't ignore her, guys!",
            sender: "wego-bot",
            senderName: "WeGo Bot"
        )
        chatSession.messages.append(introMsg)
        tableView.reloadData()
        scrollToBottom(animated: true)
        
        // 2️⃣ Wait 5 seconds, then trigger your poll test
        DispatchQueue.main.asyncAfter(deadline: .now() + 6) {
            self.showPollTest()
            
            // Step 3️⃣: After 7s total, show results summary
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                let resultsMsg = ChatMessage(
                    text: "Mexican has 2 votes, Thai has 5 votes, Fast food has 1 vote, and Chinese has 1 vote. Looks like Thai food is the champion!",
                    sender: "wego-bot",
                    senderName: "WeGo Bot"
                )
                self.chatSession.messages.append(resultsMsg)
                self.tableView.reloadData()
                self.scrollToBottom(animated: true)
                
                // Step 4️⃣: After 8s total, Sarah responds
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    let sarahMsg = ChatMessage(
                        text: "Ok that’s cool. Let’s go with Thai food.",
                        sender: "sarah",
                        senderName: "Sarah"
                    )
                    self.chatSession.messages.append(sarahMsg)
                    self.tableView.reloadData()
                    self.scrollToBottom(animated: true)
                    
                    // Step 5️⃣: Trigger the Thai poll right after Sarah's message
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                        let thaiPollMsg = ChatMessage(
                            text: "POLL_THAI", // this triggers your Thai poll cell
                            sender: "wego-bot",
                            senderName: "WeGo Bot"
                        )
                        self.chatSession.messages.append(thaiPollMsg)
                        self.tableView.reloadData()
                        self.scrollToBottom(animated: true)
                    }
                    DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                        let botReaction = ChatMessage(
                            text: "Thai Fusion has the most votes! Who is ready to devour some good food.",
                            sender: "wego-bot",
                            senderName: "WeGo Bot"
                        )
                        self.chatSession.messages.append(botReaction)
                        self.tableView.reloadData()
                        self.scrollToBottom(animated: true)
                        
                        // Step 7️⃣: Liam replies
                        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                            let liamMsg = ChatMessage(
                                text: "Bro this thing just did all the work 😂",
                                sender: "liam",
                                senderName: "Liam"
                            )
                            self.chatSession.messages.append(liamMsg)
                            self.tableView.reloadData()
                            self.scrollToBottom(animated: true)
                        }
                    }
...
class ChatMessageCell: UITableViewCell {
    ...
    func configure(with message: ChatMessage, isCurrentUser: Bool) {
        ...
        if message.sender == "wego-bot" {
            bubbleView.backgroundColor = UIColor.systemYellow.withAlphaComponent(0.85)
